...
const switchView = (viewName) => {
  ...
  // Sync sticky mobile bottom navigation active state
  document.querySelectorAll("#mobile-bottom-nav .mb-nav-item").forEach(item => {
    item.classList.toggle("active", item.getAttribute("data-view") === viewName);
  });

  // Close mobile drawer and release body scroll lock
...
const setMobileNavActive = (btnEl) => {
  document.querySelectorAll("#mobile-bottom-nav .mb-nav-item").forEach(item => item.classList.remove("active"));
  if (btnEl) btnEl.classList.add("active");
};

const handleMobileAccountClick = () => {
  if (currentUser) {
    if (typeof isInternalUser === 'function' && isInternalUser(currentUser)) {
      switchView('admin');
    } else {
      switchView('orders');
    }
...
const updateWishlistBadge = () => {
  ...
  const mbWishlistBadge = document.getElementById("mb-wishlist-badge");
  if (mbWishlistBadge) {
    mbWishlistBadge.textContent = list.length;
    mbWishlistBadge.style.display = list.length > 0 ? "inline-flex" : "none";
  }
};
...
const renderWishlistDrawer = () => {
  ...
  const wishlistedProds = list.map(id => {
    const raw = products.find(p => p.id === id);
    return raw ? (typeof getLocalizedProduct === 'function' ? getLocalizedProduct(raw, currentLang) : raw) : null;
  }).filter(Boolean);
    ...
    <div class="wishlist-item-row">
      ...
      <div class="wishlist-item-info">
        ...
        <div class="wishlist-item-actions">
          ...
          <button class="wishlist-remove-btn" onclick="handleToggleWishlist('${p.id}')" title="${t('cart_remove')}">
...
const renderProducts = () => {
  ...
  sorted.forEach(product => {
    const locProd = typeof getLocalizedProduct === 'function' ? getLocalizedProduct(product, currentLang) : product;
    ...
    if (isB2B && product.wholesalePrice) {
      priceHtml = `
        <div class="product-card-b2b-price-group">
          ...
          <span class="b2b-tag">${t('wholesale_price_tag')}</span>
        </div>
      `;
    }
    ...
    card.innerHTML = `
      <div class="product-card-img-wrapper" onclick="openProductDetail('${product.id}')">
        <img class="product-card-img" id="card-img-${product.id}" src="${activeImgSrc}" alt="${locProd.name}" onerror="this.src='images/hero_banner.png'">
        <div class="card-badges-top-left">
          ${coldChainBadge}
          ${badgeCustom}
          ${badgeOos}
        </div>
        <button class="card-wishlist-btn ${isWish ? 'active' : ''}" data-id="${product.id}" onclick="event.stopPropagation(); handleToggleWishlist('${product.id}')" title="${t('nav_wishlist')}">
      ...
      <div class="product-card-content">
        ...
        <h3 class="product-card-title" onclick="openProductDetail('${product.id}')">${locProd.name}</h3>
        ${priceHtml}
...
const renderProductDetailPage = () => {
  ...
  const product = typeof getLocalizedProduct === 'function' ? getLocalizedProduct(currentDetailProduct, currentLang) : currentDetailProduct;
  const isB2B = currentUser && currentUser.role === 'b2b_clinic';
  ...
  if (isB2B && product.wholesalePrice) {
    priceDetailHtml = `
      <div class="product-detail-price-b2b">
        <div class="price-b2b-val">${formatVND(product.wholesalePrice)} <span class="badge-b2b-price">${t('wholesale_price_tag')}</span></div>
  ...
  productDetailPageContent.innerHTML = `
    ...
    <div class="product-detail-info-col">
      ...
      <div class="detail-stock-meta">
        ...
        <span>•</span>
        <span>${t('lbl_batch')}: <strong>${batch.batchNo}</strong></span>
        <span>•</span>
        <span>${t('lbl_exp')}: <strong>${batch.expDate}</strong></span>
      </div>
          ...
          <div id="detail-tab-specs" class="detail-tab-content-pane ${currentDetailTab === 'specs' ? 'active' : ''}">
            <table class="medical-specs-table">
              <tbody>
                ...
                <tr>
                  <th>${t('spec_batch')}</th>
                  <td>${t('lbl_batch')}: <strong>${batch.batchNo}</strong> (${t('lbl_exp')}: ${batch.expDate})</td>
                </tr>
              </tbody>
            </table>
          </div>
...
const initVerifyView = () => {
  ...
  const performVerify = (codeToTest) => {
    ...
    if (!code) {
      showToast(t('serial_input_required'));
      return;
    }
      ...
      if (result) {
        ...
        verifyResultCard.innerHTML = `
          ...
          <div class="verify-detail-body">
            <p><strong>${t('lbl_serial')}:</strong> <span class="serial-code-text">${result.serial}</span></p>
            <p><strong>${t('lbl_product')}:</strong> <strong>${result.productName}</strong></p>
            <p><strong>${t('lbl_batch')}:</strong> ${result.batchNo} | <strong>${t('lbl_exp')}:</strong> ${result.expDate}</p>
            <p><strong>${t('lbl_origin')}:</strong> ${result.origin}</p>
            <p><strong>${t('lbl_license')}:</strong> ${result.license}</p>
            <p><strong>${t('lbl_distributor')}:</strong> ${result.distributor}</p>
            <div class="verify-scan-count">
              <i class="fa-solid fa-shield-halved"></i> ${t('lbl_scan_count')}: <strong>${result.checkCount}</strong>. ${t('lbl_genuine_verified')}.
            </div>
          </div>
        `;
      } else {
        const notFoundMsg = (t('serial_not_found_msg') || 'Mã serial "<strong>{code}</strong>" không tìm thấy.').replace('{code}', code);
        ...
        verifyResultCard.innerHTML = `
          ...
          <div class="verify-detail-body">
            <p>${notFoundMsg}</p>
            <p style="color:#C0392B;">${t('serial_warning_msg')}</p>
          </div>
        `;
      }
...
const setLanguage = (lang) => {
  ...
  if (metaDesc && tr['page_desc']) metaDesc.setAttribute('content', tr['page_desc']);
  ...
  if (typeof updateAdminStaffBanner === 'function') updateAdminStaffBanner();
...
// --- MOBILE TESTIMONIAL CAROUSEL CONTROLLER ---
const initTestimonialCarousel = () => {
  ...
  carousel.addEventListener("scroll", () => {
    ...
    const cardWidth = cards[0] ? cards[0].offsetWidth + 14 : 290;
