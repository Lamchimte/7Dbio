...
const updateAdminStaffBanner = () => {
  ...
  if (avatarEl) avatarEl.textContent = currentUser.avatar || (currentUser.name ? currentUser.name.charAt(0) : "7D");
  if (nameEl) nameEl.textContent = currentUser.name;
  if (titleEl) titleEl.textContent = currentUser.title || "Chuyên viên vận hành hệ thống";
  if (deptEl) deptEl.innerHTML = `<i class="fa-solid fa-building-shield"></i> ${currentUser.department || "Khối Y Dược & Vận Hành 7Dbio"}`;

  const role = currentUser.role;
  if (badgeEl && levelEl) {
    if (role === 'super_admin' || role === 'admin') {
      badgeEl.className = "badge-role badge-super-admin";
      badgeEl.innerHTML = '<i class="fa-solid fa-crown"></i> SUPER ADMIN';
      levelEl.textContent = "CẤP 5";
    } else if (role === 'warehouse_logistics') {
      badgeEl.className = "badge-role badge-warehouse";
      badgeEl.innerHTML = '<i class="fa-solid fa-snowflake"></i> KHO LẠNH GSP';
      levelEl.textContent = "CẤP 4";
    } else if (role === 'sales_rep') {
      badgeEl.className = "badge-role badge-sales";
      badgeEl.innerHTML = '<i class="fa-solid fa-briefcase"></i> SALES B2B';
      levelEl.textContent = "CẤP 3";
    }
  }
};
...
const renderAdminProducts = () => {
  ...
  products.forEach(p => {
    ...
    const catLabel = p.category === 'filler' ? t('cat_filler') : p.category === 'botox' ? t('cat_botox') : t('cat_skinbooster');
    const batchNo = p.batchInfo ? p.batchInfo.batchNo : '—';
    const wholesale = p.wholesalePrice ? formatVND(p.wholesalePrice) : '—';
    ...
    if (isSuperAdmin) {
      ...
      actionBtns = `
        <button class="admin-table-btn admin-table-btn-edit" onclick="openAdminProductModal('${p.id}')"><i class="fa-solid fa-snowflake"></i> Xem Lô/Hạn</button>
      `;
    } else if (isSales) {
      actionBtns = `
        <button class="admin-table-btn admin-table-btn-edit" onclick="openAdminProductModal('${p.id}')"><i class="fa-solid fa-eye"></i> Báo giá Sỉ</button>
      `;
    } else {
    ...
    tr.innerHTML = `
      <td><img src="${p.image}" alt="${p.name}" class="admin-product-thumb" onerror="this.src='images/hero_banner.png'"></td>
      <td>
        <div style="font-weight:600; color:var(--text-dark);">${p.name}</div>
        <div style="font-size:11px; color:var(--text-muted);">Lô: <strong>${batchNo}</strong> | Sỉ VIP: <strong>${wholesale}</strong></div>
      </td>
      <td><span style="text-transform:uppercase; font-size:11px; font-weight:600;">${catLabel}</span></td>
      <td>${formatVND(p.price)}</td>
      <td style="font-weight:600; color:${p.stock === 0 ? '#C0392B' : 'inherit'}">${p.stock}</td>
      <td>
  ...
  sortedOrders.forEach(o => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-transform:uppercase; font-size:11px; font-weight:600;">
        ${paymentText}
        ${o.isPaid ? '<span style="display:block; color:#10b981; font-size:10px;">Đã thanh toán</span>' : ''}
      </td>
      <td>
...
const viewAdminAlertEmail = (requestId) => {
  ...
  if (bodyEl) {
    bodyEl.innerHTML = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; padding: 24px;">
        <!-- Header -->
        <div style="border-bottom: 2px solid #C5A059; padding-bottom: 16px; margin-bottom: 24px; display:flex; justify-content:space-between; align-items:center;">
          <div>
            <h2 style="margin: 0; font-family: 'Playfair Display', serif; font-size: 22px; color: #000; letter-spacing: 1px;">7Dbio<span style="color:#C5A059;">.</span> VIETNAM</h2>
            <div style="font-size: 11px; color: #64748b; letter-spacing: 0.5px;">MEDICAL AESTHETICS & BIOTECHNOLOGY DISPATCH</div>
          </div>
          <span style="background: #fef3c7; color: #d97706; border: 1px solid #fde68a; font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 2px;">
        ...
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 4px; padding: 18px; margin-bottom: 24px;">
          <h4 style="margin: 0 0 12px; font-size: 13px; color: #0f172a; text-transform: uppercase; letter-spacing: 0.5px;">
            <i class="fa-solid fa-address-card" style="color:#C5A059; margin-right:6px;"></i>Thông Tin Hồ Sơ Đăng Ký
          </h4>
          <table style="width: 100%; font-size: 13px; border-collapse: collapse;">
            <tr style="border-bottom: 1px solid #edf2f7;">
              <td style="padding: 8px 0; color: #64748b; width: 140px;">Mã Request:</td>
              <td style="padding: 8px 0; font-weight: 700; font-family: monospace; color:#0284c7;">${req ? req.id : requestId}</td>
            </tr>
            <tr style="border-bottom: 1px solid #edf2f7;">
              <td style="padding: 8px 0; color: #64748b;">Họ và Tên:</td>
              <td style="padding: 8px 0; font-weight: 700; color: #0f172a;">${emailRecord.customerName}</td>
            </tr>
            <tr style="border-bottom: 1px solid #edf2f7;">
              <td style="padding: 8px 0; color: #64748b;">Email đăng nhập:</td>
              <td style="padding: 8px 0; font-weight: 600; color: #0f172a;">${emailRecord.customerEmail}</td>
            </tr>
            <tr style="border-bottom: 1px solid #edf2f7;">
              <td style="padding: 8px 0; color: #64748b;">Số điện thoại:</td>
              <td style="padding: 8px 0; font-weight: 600; color: #0f172a;">${emailRecord.customerPhone}</td>
            </tr>
            <tr style="border-bottom: 1px solid #edf2f7;">
              <td style="padding: 8px 0; color: #64748b;">Nhu cầu / Đơn vị:</td>
              <td style="padding: 8px 0; color: #334155;">${emailRecord.notes}</td>
            </tr>
            <tr>
              <td style="padding: 8px 0; color: #64748b;">Thời gian đăng ký:</td>
              <td style="padding: 8px 0; color: #64748b;">${new Date(emailRecord.sentAt).toLocaleString('vi-VN')}</td>
            </tr>
          </table>
        </div>

        <!-- Call to action inside email -->
        <div style="background: #fafaf9; border-left: 3px solid #C5A059; padding: 14px 18px; margin-bottom: 24px; font-size: 12px; color: #44403c;">
        ...
        ${req && req.status === 'pending' ? `
          <div style="text-align: center; margin-bottom: 24px;">
            <button type="button" class="btn-elegant btn-elegant-primary" onclick="closeEmailPreviewModal(); handleApproveCustomer('${req.id}');" style="padding: 12px 30px; font-size: 13px; letter-spacing: 0.5px;">
              <i class="fa-solid fa-check" style="margin-right:6px;"></i> Xác Thực &amp; Kích Hoạt Tài Khoản Này Ngay
            </button>
          </div>
        ` : `
          <div style="text-align:center; padding:12px; background:#ecfdf5; border:1px solid #a7f3d0; color:#059669; font-size:12px; font-weight:600; margin-bottom:20px;">
            <i class="fa-solid fa-circle-check"></i> Tài khoản này đã được xác thực trước đó (${req?.reviewedBy || 'Admin'}).
          </div>
        `}

        <div style="border-top: 1px solid #e2e8f0; padding-top: 16px; font-size: 11px; color: #94a3b8; text-align: center; line-height: 1.6;">
...
const renderAdminInventory = () => {
  ...
  products.forEach(p => {
    const tr = document.createElement("tr");
    const sku = p.sku || `7D-${p.id}`;
    const storage = p.medicalSpecs?.storageTemp || "2°C - 8°C (Cold-Chain)";
    const batchNo = p.batchInfo?.batchNo || "VB2026A";
    const expDate = p.batchInfo?.expDate || "2028-01-01";
    ...
    let expBadge = '';
    if (diffDays > 365) {
      expBadge = `<span class="badge-exp-safe"><i class="fa-solid fa-shield-check"></i> ${expDate} (${diffDays} ngày)</span>`;
    } else if (diffDays > 90) {
      expBadge = `<span class="badge-exp-safe" style="background:#fef3c7; color:#d97706; border-color:#fde68a;"><i class="fa-solid fa-clock"></i> ${expDate} (${diffDays} ngày)</span>`;
    } else {
      expBadge = `<span class="badge-exp-warn"><i class="fa-solid fa-triangle-exclamation"></i> ${expDate} (Gần hết hạn)</span>`;
    }
    ...
    tr.innerHTML = `
      ...
      <td>
        <div style="font-weight:600; color:var(--text-dark);">${p.name}</div>
        <div style="font-size:11px; color:var(--text-muted);">${p.medicalSpecs?.activeIngredient || ''}</div>
      </td>
      <td>
      ...
      <td>${expBadge}</td>
      <td>
        <span class="stock-pill-val ${p.stock <= 5 ? 'stock-critical' : ''}">${p.stock} sản phẩm</span>
      </td>
      <td>
        <div class="stock-control-group">
          <button class="btn-stock-pill" onclick="handleAdjustStock('${p.id}', -1)" title="Giảm 1">-</button>
          <span class="stock-num-badge">${p.stock}</span>
...
const renderAdminStaff = () => {
  ...
  staff.forEach(u => {
    ...
    if (u.role === 'super_admin' || u.role === 'admin') {
      roleBadge = `<span class="badge-role badge-super-admin"><i class="fa-solid fa-crown"></i> SUPER ADMIN</span> <span class="badge-level">CẤP 5</span>`;
      permPills = `<span class="perm-pill perm-all"><i class="fa-solid fa-shield-halved"></i> Toàn quyền hệ thống &amp; Phê duyệt tối cao</span>`;
    } else if (u.role === 'warehouse_logistics') {
      roleBadge = `<span class="badge-role badge-warehouse"><i class="fa-solid fa-snowflake"></i> KHO LẠNH GSP</span> <span class="badge-level">CẤP 4</span>`;
      permPills = `
        <span class="perm-pill"><i class="fa-solid fa-temperature-arrow-down"></i> Chuỗi lạnh 2-8°C</span>
        <span class="perm-pill"><i class="fa-solid fa-boxes-stacked"></i> Tồn kho &amp; Lô/Hạn</span>
        <span class="perm-pill"><i class="fa-solid fa-qrcode"></i> Mã chống giả</span>
        <span class="perm-pill"><i class="fa-solid fa-truck-fast"></i> Vận đơn lạnh</span>
      `;
    } else if (u.role === 'sales_rep') {
      roleBadge = `<span class="badge-role badge-sales"><i class="fa-solid fa-briefcase"></i> SALES B2B</span> <span class="badge-level">CẤP 3</span>`;
      permPills = `
        <span class="perm-pill"><i class="fa-solid fa-user-doctor"></i> Thẩm định CCHN B2B</span>
        <span class="perm-pill"><i class="fa-solid fa-tags"></i> Kích hoạt giá sỉ VIP</span>
        <span class="perm-pill"><i class="fa-solid fa-cart-flatbed"></i> Đặt hàng đại lý</span>
        <span class="perm-pill"><i class="fa-solid fa-chart-line"></i> CSKH B2B</span>
      `;
    }

    tr.innerHTML = `
      <td>
        <div class="staff-table-avatar">${u.avatar || (u.name ? u.name.charAt(0) : "7D")}</div>
      </td>
      <td>
        <strong style="font-size:14px; color:var(--text-dark);">${u.name}</strong>
      </td>
      <td>
        <div style="font-weight:600; font-size:12px; color:var(--color-primary);">${u.title || roleDef.label}</div>
        <div style="font-size:11px; color:var(--text-muted);">${u.department || roleDef.department}</div>
      </td>
      <td>${roleBadge}</td>
      <td>
        <div style="font-size:12px;"><i class="fa-solid fa-envelope" style="font-size:10px; color:var(--text-muted);"></i> <strong>${u.email}</strong></div>
        <div style="font-size:11px; color:var(--text-muted);"><i class="fa-solid fa-phone" style="font-size:10px;"></i> ${u.phone || '0988.xxx.xxx'}</div>
      </td>
      ...
      <td>
        <span class="staff-status-active"><i class="fa-solid fa-circle" style="color:#10b981; font-size:8px;"></i> Đang hoạt động</span>
      </td>
    `;
    tbody.appendChild(tr);
  });
